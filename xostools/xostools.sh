...
EOF
  ...
  while read line; do
    ...
    git reset --hard mainstream/$(echo $revision | sed -re 's/^refs\/heads\/(.*)$/\1/') || git reset --hard $remote$revision 2> /dev/null || ( [ "$repo_name" != "aosp" ] && git reset --hard mainstream/$revision ) 2> /dev/null || git reset --hard $revision 2> /dev/null || git reset --hard
    git clean -fdx || :
    popd
    echo
  done < <(repo list)
